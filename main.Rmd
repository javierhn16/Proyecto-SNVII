---
title: "Proyecto - Seguros de no Vida II"
author: 
...
fig <- ggplot(data, aes(x = log(AggClaim))) +
  geom_density(color = "lightblue", linewidth = 1.3,
               adjust = 1.2, fill = "lightblue") +
...
fig <- ggplot(data, aes(x = log(AggClaim), y = OpTime)) +
  ...
  scale_fill_manual(
    values = colorRampPalette(c("#02102a", "#3182bd",
                                "#fef0d9", "gray")[4:1])(13)
  ) +
  labs(
...
triangulo <- pivot_wider(datosm, names_from = k,
                         values_from = inc, values_fill = 0) %>%
  arrange(ocurrencia)
triangulo <- data.frame(triangulo)
rownames(triangulo) <- triangulo[, 1]  
triangulo <- triangulo[, -1]           
colnames(triangulo) <- 0:max(datosm$k, na.rm = TRUE)
```
...
triangle_to_df(tria) %>%
  kable(format = "html", caption = "Triángulo de Montos de Siniestros por CL") %>%
  kable_styling(
  ...
  column_spec(1:ncol(triangulo), width = "4em")
...
rownames(triangulo) <- triangulo[, 1]
triangulo <- triangulo[, -1]
colnames(triangulo) <- 0:max(datosc$k, na.rm = TRUE)
```
...
tria <- CL$FullTriangle
triangle_to_df(tria) %>%
  kable(format = "html", caption = "Triángulo de Cantidad de Siniestros por CL") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width = FALSE,
    position = "center"
...
prepare_ay_dy <- function(data) {
  ...
  DY_cont <- as.numeric(difftime(data$ReportDate,
                                 data$AccDate, units = "days")) / 365
...
# 2) Ajustar cópula
fit_copula_pobs <- function(AY_cont, DY_cont,
...
# 3) Estimar probabilidades condicionales
...
# 4) Construir triángulo de probabilidades
...
# 5) Condensar la información en una sola lista
estimate_copula_runoff <- function(data,
  ...
  probs <- estimate_conditional_probs(data_p, fit_obj,
                                      simulate_n = simulate_n,
                                      years = years, max_lag = max_lag,
                                      seed = seed)
  tri_out <- build_triangle_from_probs(data_p, probs, years = years,
                                       max_lag = max_lag)
...
```{r}
out <- estimate_copula_runoff(data, family = "frank", 
                              simulate_n = 200000, max_lag = 5)
```
...
colnames(final) <- 0:5
final %>%
  kable(format = "html", caption = "Triángulo de Montos de Siniestros por Cópulas") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width = FALSE,
    position = "center"
...
## Por cantidades de siniestros
```{r}
final <- t(apply(out$triangle_expected_counts, 1, cumsum))  
colnames(final) <- 0:5
final %>%
  kable(format = "html", caption = "Triángulo de Cantidades de Siniestros por Cópulas") %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width = FALSE,
    position = "center"
